  ...
  const sources = [
    {
      ...
      is_active: 1, is_default: 1, ingestion_method: 'N8N', status: 'placeholder',
    },
    {
      ...
      base_url: 'https://www.empleosperu.gob.pe', is_active: 1, is_default: 1,
    ...
    },
    {
      ...
      base_url: 'https://www.pj.gob.pe', is_active: 1, is_default: 1,
      ingestion_method: 'N8N', status: 'placeholder',
    },
    {
      key: 'computrabajo', name: 'Computrabajo', type: 'private',
      description: 'Bolsa de trabajo privada (no integrada todavía; placeholder).',
      base_url: 'https://www.computrabajo.com.pe', is_active: 1, is_default: 0,
    ...
    },
    {
      ...
      base_url: 'https://pe.indeed.com', is_active: 1, is_default: 0,
      ingestion_method: 'SCRAPING', status: 'placeholder',
    },
    {
      key: 'linkedin', name: 'LinkedIn', type: 'private',
      description: 'Bolsa de trabajo profesional (no integrada todavía; placeholder).',
      base_url: 'https://www.linkedin.com/jobs', is_active: 1, is_default: 0,
      ingestion_method: 'SCRAPING', status: 'placeholder',
    },
    {
      ...
      base_url: 'https://www.yaempleo.net', is_active: 1, is_default: 0,
      ingestion_method: 'SCRAPING', status: 'placeholder',
    },
  ];
  sources.forEach((s) => sourcesRepository.create(s));
  console.log(`[seed] ${sources.length} fuentes creadas`);
}

function S(sourceKey) {
  const row = sourcesRepository.findByKey(sourceKey);
  if (!row) throw new Error(`Fuente ${sourceKey} no encontrada`);
  return row.id;
}

function seedOffers(sourceIds) {
  const offers = [
    {
  ...
  offers.forEach((o) => {
    offersRepository.upsert({
      source_id: sourceIds[byKey[o.external_id]],
      ...
      duracion: o.duration,
      penalizacion: o.penalization ? 1 : 0,
      fuente: byKey[o.external_id],
  ...
  console.log(`[seed] ${offers.length} ofertas de prueba creadas (todas marcadas es_demo=1)`);
}

function seedDemoUser() {
  const { user } = registerUser({ email: 'prueba@demo.com', password: DEMO_PASSWORD });
  ...
  const allSources = sourcesRepository.listAll().map((s) => s.id);
  userSourcesRepository.setEnabled(user.id, allSources);
...
export async function runSeed() {
  ...
  seedSources();
  const sourceIds = {
    'servir-ofertas': S('servir-ofertas'),
    'empleos-peru': S('empleos-peru'),
    'poder-judicial': S('poder-judicial'),
    'computrabajo': S('computrabajo'),
    'indeed': S('indeed'),
    'linkedin': S('linkedin'),
    'yaempleo': S('yaempleo'),
  };
  seedOffers(sourceIds);
  const userId = seedDemoUser();
  await seedMatches(userId);

  const enabled = userSourcesRepository.enabledSourceIds(userId);
