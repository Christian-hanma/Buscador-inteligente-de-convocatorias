...
export async function generateAdaptedCv(userId, offerId) {
  ...
  const version = cvsRepository.nextVersion(userId, offerId);
  const generated = cvsRepository.create({
    userId,
    offerId,
...
export function listCvs(userId) {
  return cvsRepository.listForUser(userId);
}

export function getCv(userId, id) {
  const cv = cvsRepository.findByIdForUser(userId, id);
